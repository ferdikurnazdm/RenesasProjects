...
#if 0xFF != 0x03
    .de_control_pin = BSP_IO_PORT_03_PIN_10,
#else
...
/* Actual period: 0.5 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x3938700,
  .duty_cycle_counts = 0x1c9c380, .source_div = (timer_source_div_t) 0, .channel = 5, .p_callback = NULL,
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.5 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x3938700,
  .duty_cycle_counts = 0x1c9c380, .source_div = (timer_source_div_t) 0, .channel = 2, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
